...
function ReportTimeStamp(props) {
  ...
  useEffect(() => {
    if (data.current.length === 2) addStampData(1, data.current[0], data.current[1], hour, index);
    else addStampData(0, 0, 0, 0, 0)
  }, [hour, index, addStampData]);
...
export default ReportTimeStamp;
